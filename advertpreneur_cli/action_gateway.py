...
class ExternalActionGateway:
    ...
    def contract() -> str:
        return (
            ...
            "Use browser actions yourself for live panels. Never ask for credentials. When the user explicitly asks to use a saved browser session and the observed login form is already autofilled, click its observed Login control once and verify the result. If credentials are empty, or the page requires MFA, CAPTCHA, or another verification, report `Login needed in browser` and stop. "
