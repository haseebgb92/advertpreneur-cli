...
class ExternalActionGateway:
    ...
    def contract() -> str:
        return (
            ...
            "Use browser actions yourself for live panels. For normal navigation, call browser `visual_context` at each new page or modal, use its observed control labels/selectors for one next action, and call it again to verify the resulting state. It is compact; reserve `inspect` for a targeted structural question and `reverse_engineer` for explicit design work. Never ask for credentials. When the user explicitly asks to use a saved browser session and the observed login form is already autofilled, click its observed Login control once and verify the result. If credentials are empty, or the page requires MFA, CAPTCHA, or another verification, report `Login needed in browser` and stop. "
            "When a named taught browser routine matches, reuse it before visual rediscovery; provide only new task parameters and preserve protected tabs. "
            "For Amazon/Helium 10 keyword research, use browser tab slots: `access` for Softzilla, capture Launch Web App into `helium`, and keep searches in `amazon`. Before the first keyword, use `visual_context` on Amazon's delivery location and, only when it is not New York ZIP 10001, use observed controls to set and confirm 10001. Create one local run with browser `research_start`. Use `visual_context` on the active Amazon/Xray UI and pass only observed selectors for search, submit, open, rows, load_more, refresh, export, and csv as JSON to `research_xray_setup`; never invent static selectors. Then use `research_run`: it opens Xray, uses Load more only after observed row growth, refreshes Xray once after a stall, exports through the observed CSV control, records each verified download, and refreshes only the Amazon tab. "
