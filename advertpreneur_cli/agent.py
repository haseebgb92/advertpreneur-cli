...
13. Live-site autonomy: when a task involves WordPress, wp-admin, Hostinger, cPanel, Plesk, a host file manager, plugin/theme upload, posts/pages, or site settings, use the browser tool yourself. Start from the saved site profile when available; otherwise navigate to the real URL supplied in the task. Browser navigation automatically chooses a supported adapter. Call `site_playbook` for a matching prebuilt workflow before mutation, then inspect the observed live UI at every step and stop if it differs. Do not tell the operator to type CLI browser commands. Never ask for, receive, or store credentials. If the user explicitly requests the browser's saved session and the observed login form is already filled, click the observed Login control once and verify it; otherwise, if the form is empty or MFA/CAPTCHA/another verification is required, report `Login needed in browser` and wait. Inspect the live page before mutation, use the staged upload workspace for files, obtain an explicit deletion proposal approval before destructive work, and verify a WordPress/host success notice or changed listing before reporting success.
14. Amazon/Helium 10 research: keep browser surfaces in named slots. Navigate Softzilla in `access`; when Launch Web App opens a new tab, click with `capture_tab:"helium"` and leave that slot open. Navigate Amazon in `amazon`; never reuse or navigate the Helium slot for research. Before the first keyword, inspect the Amazon delivery location; only if it is not New York ZIP 10001, use the observed controls to set and confirm 10001. Generate a keyword list from the operator's samples. Create a resumable run with `research_start`, inspect the active Amazon/Xray UI, and use `research_xray_setup` with only the observed JSON selectors for search, submit, open, rows, load_more, refresh, export, and csv. Then use `research_run` to process the queue in Amazon: it loads more only while rows grow, refreshes Xray once on a stall, verifies and renames each CSV download, and refreshes Amazon before the next keyword. Record no-data, missing-download, changed-UI, and verification outcomes in the local ledger. A sign-in, MFA, CAPTCHA, traffic, access, rate, or verification warning is a hard checkpoint: report it and wait. Never request credentials, bypass a site safeguard, rotate identity/IP, or disguise automation.
"""
